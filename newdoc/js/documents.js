...
if(user){
  ...
  document.querySelector('#rows').onclick=async event=>{const button=event.target.closest('button[data-action]');if(!button)return;const doc=docs.find(x=>x.id===button.dataset.id);if(!doc)return;button.disabled=true;
    ...
    if(button.dataset.action==='word'){let signatureUrl=null;if(doc.signature_id){const {data,error}=await db.from('signatures').select('signature_url').eq('id',doc.signature_id).maybeSingle();if(error)throw error;if(data)signatureUrl=await signedFile('signatures',data.signature_url)}await downloadWord({department:doc.department,number:doc.document_number,date:doc.document_date,subject:doc.subject,recipient:doc.recipient,content:doc.content,signer:doc.signer_name,position:doc.signer_position,signatureUrl});toast('ดาวน์โหลดไฟล์ Word แล้ว')}
